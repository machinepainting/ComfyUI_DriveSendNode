#!/usr/bin/env python3
"""
DriveSend Cross-Platform Decryption Script
Works on macOS, Windows, and Linux

============================================================
LOCAL USE ONLY - Run this on your local machine to decrypt
.enc files downloaded/synced from Google Drive. Do not run on
cloud instances (RunPod, etc.) - decryption happens locally
where you want to view your files.
============================================================

Key retrieval methods:
  - DRIVESEND_ENCRYPTION_KEY environment variable (all platforms)
  - macOS Keychain
...
def get_key_from_macos_keychain():
    ...
    try:
        ...
        result = subprocess.run(
            ['security', 'find-generic-password', '-s', 'DriveSend_Encryption_Key', '-a', 'DriveSend', '-w'],
...
def get_key_from_linux_secret_service():
    """Try to retrieve the key from Linux Secret Service (GNOME Keyring/KWallet)."""
    try:
        import subprocess
        result = subprocess.run(
            ['secret-tool', 'lookup', 'service', 'DriveSend', 'username', 'DriveSend'],
            capture_output=True, text=True
        )
        if result.returncode == 0 and result.stdout.strip():
            return result.stdout.strip()
...
def get_encryption_key():
    ...
    # Check both DRIVESEND and comfyui variants
    key = os.environ.get('DRIVESEND_ENCRYPTION_KEY') or os.environ.get('comfyui_encryption_key')
    if key:
    ...
    if system == 'Darwin':  # macOS
        ...
        if key:
            print("Using key from Secret Service (GNOME Keyring/KWallet).")
            return key
    ...
    if system == 'Darwin':
        ...
        print("    Item Name: DriveSend_Encryption_Key, Account: DriveSend")
    elif system == 'Windows':
        print("  - Set environment variable DRIVESEND_ENCRYPTION_KEY")
        print("    (Win+R > sysdm.cpl > Advanced > Environment Variables)")
    elif system == 'Linux':
        print("  - Set environment variable: export DRIVESEND_ENCRYPTION_KEY=\"your_key\"")
        print("  - Or use secret-tool: echo -n \"key\" | secret-tool store --label=\"DriveSend\" service DriveSend username DriveSend")
    print("")
...
def main():
    system = platform.system()
    print(f"=== DriveSend File Decryption ({system}) ===")
    print("")
    print("Decrypts .enc files in a folder to restore original images/videos.")
    print("")
    
    # Get folder path
    folder = input("Enter the folder path containing .enc files: ").strip()
    # Remove surrounding quotes if present (common when drag-dropping on Windows)
    folder = folder.strip('"\'')
    # Handle escaped spaces from drag-drop on macOS
    ...
    for enc_path in enc_files:
        ...
        # e.g., image.png.enc -> image.png, video.mp4.enc -> video.mp4
